...
public class TransactionServiceImpl implements TransactionService {
    ...
    public Transaction create(Transaction value) {                //  1. 	Получаем информацию о каждой расходной операции
        ...
        MonthLimit limit = monthLimitRepository.findActualLimitByCategory(value.getExpenseCategory());              // 2. Если лимит не установлен, создаем дефолтный лимит с суммой == 0;
        Double actualRate = currencyRatesClient.getActualRate((value.getDateTime()));
        Double sumTransactionsInLimitPeriodInUSD = monthLimitRepository.sumTransactionFromLimitPeriod(value.getExpenseCategory()) / actualRate;
    ...
    public Map<Transaction, LimitExceededDto> detailingReportForLimitExceededTransactions() {        //  Возвращаем список транзакций, превысивших лимит, с указанием лимита,
        ...
        for (Transaction transaction : transactions) {
            MonthLimit monthLimit = monthLimitRepository.findLimitByDate(transaction.getDateTime());
